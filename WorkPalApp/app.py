...
class App:
    ...
    def run_posture_analysis(self):
        if self.posture_analysis_var.get():
            ...
            self.posture_analysis_process = subprocess.Popen(["python", "poseAnalysis.py"])############
        else:
    ...
    def view_pose(self):
        ...
        if self.viewPose:
            self.posture_analysis_process.terminate()
            self.view_pose_process=subprocess.Popen(["python", "poseAnalysis.py","1"])############
    ...
    def generate_report(self):
        subprocess.Popen(["python", "reportGenerator.py"])
